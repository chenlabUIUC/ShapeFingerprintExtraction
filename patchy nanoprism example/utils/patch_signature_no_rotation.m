% NAME:     this function extracts the shape signature d(theta) from the 
%           patch contour coordinates of a patchy nanoparticle.
%
% INPUT:    boundary: Nx2 array containing the coordinates of all points on 
%           one patch boundary from a nanoparticle.
%           center: the geometrical centroid of the patchy nanoparticle core.
% NOTE:     this function DOES NOT shift the signature.
% OUTPUT:   out: 1x360 array of shape signature d(theta) of one polymer patch. 
%           The range of theta is [-179:180] degrees with an interval of 1 degree.
% 
% HISTORY:  Written by Lehan Yao
% Last modified by Lehan Yao on 06/07/2021
function out = patch_signature_no_rotation(boundary,center)
    y0=boundary(:,1);
    x0=boundary(:,2);
    boundary=[x0,y0];
    rads=1000;
